...
@Service
public class IB0501ServiceImpl implements B0501Service {
	@Autowired
	 ...
	 */
	@Override
	public List<TrainingInfoTblBean> getAllTrainingInitCount(B0501VueForm trainingEntity) {
		List<TrainingInfoTblEntity> trainingInfoTblEntity = trainingTblMapper.getAllTraining(trainingEntity);
		...
		for (TrainingInfoTblEntity trainingInfoEntity : trainingInfoTblEntity) {
			...
			if (trainingInfoEntity.getStatusFlg().equals("0")) {
				bean.setStatusFlg("未確認");
			} else if (trainingInfoEntity.getStatusFlg().equals("1")) {
				bean.setStatusFlg("確認済み");
			} else {
				bean.setStatusFlg("拒否する");
			}
	 ...
	 */
	@Override
	public int update(List<Integer> trainingIdList, Integer companyId, Integer userCd) {
		int updateReturn = trainingTblMapper.update(trainingIdList, companyId, userCd);
		return updateReturn;
	}
